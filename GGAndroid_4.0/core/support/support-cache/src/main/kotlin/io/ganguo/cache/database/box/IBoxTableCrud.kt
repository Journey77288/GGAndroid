...
import io.ganguo.cache.database.ITableCrud
...
interface IBoxTableCrud<T> : ITableCrud<T> {
    ...
    fun asRxQuery(queryBuilder: QueryBuilder<T>): Observable<List<T>> {
        return RxJavaBridge.toV3Observable<List<T>>(RxQuery.observable(queryBuilder.build()))
    }
}
