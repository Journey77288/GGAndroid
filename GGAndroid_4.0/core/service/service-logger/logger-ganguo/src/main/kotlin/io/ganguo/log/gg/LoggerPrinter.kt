...
open class LoggerPrinter : Printer {
    ...
    private fun format(message: String?, vararg args: Any?): String = let {
        if (message.orEmpty().isEmpty()) {
            ""
        } else {
            val msg = message?.replace("%", "%%")
            String.format(Locale.getDefault(), msg.orEmpty(), *args)
        }
    }
