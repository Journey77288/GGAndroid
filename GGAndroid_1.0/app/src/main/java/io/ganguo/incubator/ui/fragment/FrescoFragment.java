 ...
 */
public class FrescoFragment extends BaseFragment implements SwipeRefreshView.OnRefreshListener {
    ...
    protected void initData() {
        mAdapter = new CreatorAdapter(getActivity(), new IViewCreator<String>() {
            ...
            public ViewHolder createView(Context context, int position, String item) {
                return new ViewHolder(View.inflate(context, R.layout.item_fresco, null));
            }

            @Override
            public void updateView(ViewHolder viewHolder, int position, String item) {
                item = "http://ganguo.io/images/case_02.png";

                /*
                  需要将xml里的imageView替换成SimpleDraweeView
                  */

                SimpleDraweeView sdv_img = viewHolder.findViewById(R.id.sdv_img);
                GFresco.getInstance().setPlaceHolderImage(R.drawable.ic_launcher);
                GFresco.getInstance().displayImage(sdv_img, item);
            }
