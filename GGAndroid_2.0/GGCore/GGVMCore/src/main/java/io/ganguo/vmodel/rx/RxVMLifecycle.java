...
public class RxVMLifecycle {
    ...
    public static <T> ObservableTransformer<T, T> bindViewModel(final BaseViewModel viewModel) {
        return source -> {
            checkNotNull(viewModel);
            return source.takeUntil(viewModel.getLifecycleHelper().getOnDestroyBehavior());
        };
    }
    ...
    public static <T> SingleTransformer<T, T> bindSingleViewModel(final BaseViewModel viewModel) {
        return source -> {
            checkNotNull(viewModel);
            return source.takeUntil((SingleSource<BehaviorRelay<String>>) observer -> viewModel.getLifecycleHelper().getOnDestroyBehavior());
        };
    }
