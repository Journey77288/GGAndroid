 ...
 */
public class RxProperty<T> extends Observable<T> implements android.databinding.Observable, Disposable {
    ...
    public RxProperty(@NonNull T defaultValue) {
        this(Observable.<T>never(), RxHelper.createInitialMaybe(defaultValue), DEFAULT_MODE);
    }
    ...
    protected RxProperty(@NonNull Observable<T> source, @NonNull Maybe<T> initialMaybe,
                         @MODE int mode) {
        RxHelper.checkNull(source, "source");
