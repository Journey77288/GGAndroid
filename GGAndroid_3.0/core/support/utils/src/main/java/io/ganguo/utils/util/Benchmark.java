...
public class Benchmark {
    ...
    public static void end(String tag) {
        ...
        if (entry == null) {
            Logger.INSTANCE.w("Benchmark Not Match, tag spell mistake or forgot Benchmark.end(tag) invoke at somewhere ??");
            return;
        }
        entry.end = System.currentTimeMillis();

        long used = entry.end - entry.start;
        Logger.INSTANCE.d("Benchmark [ " + entry.tag + " ] - Used: " + (used) + " ms. ");
