 ...
 *
 */
abstract class SwitchButton : View {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        val defaultWidth = calculateButtonWidth()
        var defaultHeight = calculateButtonHeight()
     ...
     * @return Int
     */
    protected open fun calculateButtonHeight(): Int {
        var thumbHeight = thumbSize
        if (isEnableThumbShadow) {
            thumbHeight = thumbShadowSize + thumbShadowDy * 2
        }
        return if (thumbHeight > trackHeight) {
            thumbHeight
        } else {
            trackHeight
        }
     ...
     * @return Int
     */
    protected open fun calculateButtonWidth(): Int {
        return trackWidth
    }
     ...
     * 初始化滑块默认坐标
     */
    protected open fun initThumbCoordinate() {
        var thumbPadding = (height - thumbSize) * 0.5f
        thumbCenterY = height * 0.5f
        thumbOffCenterX = thumbPadding + thumbSize * 0.5f
        thumbOnCenterX = width - thumbPadding - thumbSize * 0.5f
        thumbTotalOffset = thumbOnCenterX - thumbOffCenterX
    }
